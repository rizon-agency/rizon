...
export function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    alternatives.map((alternative) => ({ locale, slug: alternative.slug })),
  );
}
...
export async function generateMetadata({
  ...
  const metaTitle = l(alternative.metaTitle, locale as Locale);
  const metaDescription = l(alternative.metaDescription, locale as Locale);
  return {
    title: metaTitle,
    ...
    "@graph": [
      {
        ...
        "@id": `${url}#webpage`,
        url,
        name: local(alternative.metaTitle),
        description: local(alternative.metaDescription),
        about: {
          "@type": "Thing",
      ...
      <main>
        <section className="container pt-24 md:pt-28">
          ...
          <div className="mt-10 max-w-4xl">
            <Eyebrow>{local(alternative.category)}</Eyebrow>
        ...
        <section className="container mt-24 md:mt-32">
          <div className="grid grid-cols-1 gap-12 border-t border-border pt-10 lg:grid-cols-12">
            <div className="lg:col-span-4"><span className="inline-flex items-center rounded-full bg-primary/10 px-3.5 py-1.5 text-sm font-medium text-primary">{t("intro.label")}</span></div>
            <div className="space-y-5 text-lg leading-relaxed text-muted-foreground lg:col-span-7 lg:col-start-6">
              ...
              <p className="border-l-2 border-primary pl-5 text-foreground"><strong>{t("intro.whoThisIsFor")}</strong> {local(alternative.whoThisIsFor)}</p>
            </div>
          </div>
        </section>

        {alternative.stat && <section className="container mt-16"><StatBlock stat={{ ...alternative.stat, label: local(alternative.stat.label) }} /></section>}

        <section className="container mt-24 md:mt-32">
          <div className="max-w-3xl"><span className="inline-flex items-center rounded-full bg-primary/10 px-3.5 py-1.5 text-sm font-medium text-primary">{t("whyLeave.label")}</span><h2 className="mt-5 text-4xl font-semibold tracking-tight text-balance md:text-5xl">{t("whyLeave.titleTemplate", { competitor: alternative.competitor })}</h2></div>
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-2">
            {alternative.whyLeave.map((item, index) => <article key={local(item.title)} className="surface surface-hover p-7 md:p-8"><span className="font-mono text-sm font-medium text-primary">0{index + 1}</span><h3 className="mt-5 text-2xl font-semibold tracking-tight">{local(item.title)}</h3><p className="mt-4 leading-relaxed text-muted-foreground">{local(item.body)}</p></article>)}
          </div>
        </section>

        <section id="comparison" className="container mt-24 scroll-mt-24 md:mt-32">
          <div className="max-w-3xl"><span className="inline-flex items-center rounded-full bg-primary/10 px-3.5 py-1.5 text-sm font-medium text-primary">{t("comparison.label")}</span><h2 className="mt-5 text-4xl font-semibold tracking-tight text-balance md:text-5xl">{t("comparison.titleTemplate", { competitor: alternative.competitor })}</h2></div>
          <div className="surface mt-12 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-[15px]"><thead className="bg-muted/50"><tr><th scope="col" className="w-1/4 px-5 py-4 font-medium">{t("comparison.dimension")}</th><th scope="col" className="w-[37.5%] border-l border-border px-5 py-4 font-medium">{t("comparison.rizon")}</th><th scope="col" className="w-[37.5%] border-l border-border px-5 py-4 font-medium">{alternative.competitor}</th></tr></thead><tbody>{alternative.comparison.map((row) => <tr key={local(row.dimension)} className="border-t border-border align-top"><th scope="row" className="px-5 py-5 font-medium">{local(row.dimension)}</th><td className="border-l border-border px-5 py-5 leading-relaxed text-muted-foreground">{local(row.rizon)}</td><td className="border-l border-border px-5 py-5 leading-relaxed text-muted-foreground">{local(row.them)}</td></tr>)}</tbody></table></div>
        </section>

        <section className="container mt-24 md:mt-32"><div className="grid grid-cols-1 gap-12 border-t border-border pt-10 lg:grid-cols-12"><div className="lg:col-span-4"><span className="inline-flex items-center rounded-full bg-primary/10 px-3.5 py-1.5 text-sm font-medium text-primary">{t("whereTheyWin.label")}</span><h2 className="mt-5 text-3xl font-semibold tracking-tight">{t("whereTheyWin.titleTemplate", { competitor: alternative.competitor })}</h2></div><div className="space-y-7 lg:col-span-7 lg:col-start-6">{alternative.whereTheyWin.map((item) => <article key={local(item.title)}><h3 className="text-xl font-medium tracking-tight">{local(item.title)}</h3><p className="mt-3 leading-relaxed text-muted-foreground">{local(item.body)}</p></article>)}</div></div></section>

        <section className="container mt-24 md:mt-32"><div className="rounded-2xl bg-tint px-6 py-10 md:px-10 md:py-14"><span className="inline-flex items-center rounded-full bg-primary/10 px-3.5 py-1.5 text-sm font-medium text-primary">{t("migration.label")}</span><h2 className="mt-5 max-w-2xl text-3xl font-semibold tracking-tight md:text-4xl">{t("migration.title")}</h2><p className="mt-6 max-w-3xl leading-relaxed text-muted-foreground">{local(alternative.migration)}</p>{relatedPosts.length > 0 && <div className="mt-7 flex flex-wrap gap-3">{relatedPosts.map((post) => <Button key={post.slug} variant="outline" asChild><Link href={`/blog/${post.slug}`}>{t("migration.readPrefix")}: {post.title}<ArrowRight size={15} aria-hidden /></Link></Button>)}</div>}</div></section>
