...
export const Trust = () => <section className="container mt-32 md:mt-40"><SectionHeader label="Client feedback" title="The work has to hold up after the handoff." description="From clients who brought Rizon in when the learning-platform work got hard." /><div className="mt-12 grid grid-cols-1 border-t border-border md:grid-cols-2">{testimonials.map((quote, i) => <blockquote key={quote} className="border-b border-border py-8 md:pr-10 md:odd:border-r md:odd:pr-12 md:even:pl-12"><p className="text-xl leading-relaxed tracking-tight">&ldquo;{quote}&rdquo;</p><footer className="mt-5 font-mono text-xs tabular-nums text-muted-foreground/60">{String(i + 1).padStart(2, "0")}</footer></blockquote>)}</div><div className="mt-12 border-t border-border pt-8"><p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Selected client organisations</p><div className="mt-5 flex flex-wrap gap-x-7 gap-y-3">{clients.map((client) => <Link key={client.href} href={client.href} target="_blank" rel="noopener noreferrer" className="text-lg font-medium text-muted-foreground hover:text-foreground">{client.name}</Link>)}</div></div></section>;
