...
That is why "the site loads" is not enough evidence. You need to inspect the request behind the spinner.
...
Moodle 5.2 requires `max_input_vars` of at least 5000 according to the [5.2 release notes](https://moodledev.io/general/releases/5.2). That setting is not only about file uploads, but it is a useful signal that the PHP environment has actually been prepared for the version.
