...
**Per-tenant identity.** Larger tenants will want to connect their own single sign-on, so their staff log in with their existing company accounts. Supporting a different identity setup for each tenant is a common late-stage surprise. It is worth reading alongside [SSO options for learning platforms](sso-options-for-learning-platforms.md), because in a multi-tenant system you are not solving login once, you are solving it per tenant.
...
When tenancy is central to how you sell, [custom LMS development](/services/custom-lms-development) is largely about owning that isolation, branding, and billing rather than renting a model you cannot change.
